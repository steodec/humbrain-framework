...
class QueryBuilder
{
    ...
    private function whereBuilder(): void
    {
        if (!empty($this->where)) {
            ...
            foreach ($this->where as $where) {
                $keymap = strtolower($where['field']);
                $this->query .= "{$where['field']} {$where['operator']} :{$keymap} {$where['andOrOr']} ";
            }
    ...
    private function saveQuery(): string
    {
        ...
        $this->query .= implode(', ', array_map(function ($data) {
            $datakey = strtolower($data['column']);
    ...
    final public function getValues(): array
    {
        ...
        return array_map(fn($data) => [strtolower($data['column']) => $data['value']], $data);
    }
}
